...
# Modifications:
# Section for reading data from grid file has been modified by Vilhelm
# Fagerström on January 12 2022. Modificaions in lines 168, 169 and 172-176
# Data now opened using 'open_dataset' from xarray instead of 'Dataset' from
# netCDF4
    ...
    def __init__(self, filename=None, name=None, gridfile=None):
        ...
        if 'lat_rho' in self.Dataset.variables:
            ...
            if gridfile is None:
                ...
                gf = xr.open_dataset(gridfile) # ADDED!!!
                #gf = Dataset(gridfile) # Commented out
                self.lat = gf.variables['lat_rho'][:]
                self.lon = gf.variables['lon_rho'][:]
                self.lon = self.lon.data  # Extract, could be avoided downstream ADDED!!!
                self.lat = self.lat.data # ADDED!!!
                if self.lat.ndim == 1: # ADDED!!!
                    self.lon, self.lat = np.meshgrid(self.lon, self.lat) # ADDED!!!
                    self.angle_xi_east = 0 # ADDED!!!
